<template>
  <div>
    <div class="header">
      <div class="inte">
       <div class="text">
           <h5 class="mun">{{teamAmountB== null ? '--' :parseInt(teamAmountB)}}</h5>
           <p class="title">二部总业绩</p>
       </div>
      </div>
    </div>
    <div class="cont">
  ...
  methods: {
    list (page) {
      this.$http({
        ...
        if (data.code === 'ok') {
          this.teamAmountB = data.data.teamAmountB
        }
      })
      this.$http({
      ...
      setTimeout(() => {
        ...
        if (this.hasNext === true) {
          ...
          this.$http({
            url: this.$http.adornUrl('/h5/account/fetchUserTeamAmountLogs'),
            method: 'get',
            params: {page: this.page, limit: 20, teamType: 2}
          }).then(({data}) => {
            if (data.code === 'ok') {
              for (let i = 0; i < data.data.content.length; i++) {
                data.data.content[i].occurTime = getDate(data.data.content[i].occurTime, 'yyyy-MM-dd hh:mm:ss')
...
.inte{
  width: 100%;
  height: 4.4rem;
  background: url('../../assets/yejiBig4.png') no-repeat;
